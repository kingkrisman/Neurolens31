...
export function SettingsPanel() {
  ...
  const preview =
    profile.bionicStrength > 0
      ? processBionicText(DEMO_SENTENCE, profile.bionicStrength, profile.rhythmOptimization)
      : DEMO_SENTENCE;

  return (
    <PageEnter className="mx-auto h-full max-w-3xl px-4 py-10 sm:px-8 sm:py-14">
      <h1 data-enter className="text-5xl">
        Settings
      </h1>
      ...
      <div className="mt-10">
        <ScrollScene>
          <div className="space-y-3">
            <h2 className="mt-8 mb-1 px-1 text-xs font-medium tracking-wide text-muted uppercase first:mt-0">
              Reading
            </h2>
            {/* The survey, reachable on purpose.
            It runs itself once per account, which leaves no way back to it —
            and no way to reach it at all for anybody whose account predates it.
            Five questions are a faster route to a sensible page than a panel of
            sliders, so they should not be a thing you get one chance at. */}
            <Panel>
              <PanelHeader
                as="h3"
                title="Set up my reading"
                description="Five questions, and it sets the page up for you. Shows what it will change before anything is saved."
              />
              <PanelWell className="p-2">
                <Button variant="outline" className="w-full" onClick={() => setSurveyOpen(true)}>
              ...
              <PanelWell className="space-y-5 p-3">
                ...
                <div>
                  ...
                  <Slider
                    ...
                    onValueChange={([value]) =>
                      setProfile({ ...profile, lineHeight: value ?? 1.6 })
                    }
                ...
                <div>
                  ...
                  <Slider
                    ...
                    onValueChange={([value]) =>
                      setProfile({ ...profile, letterSpacing: value ?? 0 })
                    }
              ...
              <PanelWell className="space-y-5 p-3">
                ...
                <div>
                  <div className="mb-1 flex items-center justify-between text-sm">
                    ...
                    <span className="tabular-nums text-muted">
                      {Math.round(profile.bionicStrength * 100)}%
                    </span>
                  </div>
                  <Slider
                    min={0}
                ...
                <div className="flex min-h-11 items-center justify-between gap-3 px-1 py-1">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      setProfile({ ...profile, plainLanguage: checked })
                    }
                  />
                </div>
                <div className="flex min-h-11 items-start justify-between gap-3 px-1 py-2">
                  <span className="flex min-w-0 flex-col">
                    ...
                    >
                      For motion sickness: stops parallax and scroll drift, and runs markers along
                      the screen edges that move against your travel. Uses the motion sensor where
                      one exists, so it works in a car or on a train. Separate from your system’s
              ...
              <PanelWell className="px-4 py-4">
                ...
                >
                  ...
                  <span className="relative">
                    {holding ? "Hold to confirm" : "Clear local data"}
                  </span>
                </Button>
              </PanelWell>
            </Panel>
            ...
            <Panel className="mb-8">
              ...
              <PanelWell className="px-4 py-4">
                <p className="text-sm leading-relaxed text-muted">
                  An adaptive reading environment. Paste text, open a PDF page by page, look up a
                  Bible chapter, search Project Gutenberg, or open a poem. Adaptive watches pace,
                  pauses, rereads, and feel — it recommends, and it never silently rewrites a locked
                  setting.
                </p>
      ...
      {surveyOpen ? <OnboardingSurvey onDone={() => setSurveyOpen(false)} /> : null}
    </PageEnter>
  );
}
